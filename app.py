...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/jstempelHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    update_priority = StringField("Update the prority of a TODO list item", validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        update_priority = form.update_priority.data
        my_var = TodoItem.query.filter_by(description=item).first()
        my_var.priority = update_priority
        db.session.commit()
        flash("Updated priority of item: " + my_var.description)
        return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    form = DeleteButtonForm()
    var = TodoList.query.filter_by(title = lst).first()

    db.session.delete(var)
    db.session.commit()
    flash(lst + ' has been deleted.')
    return redirect(url_for('all_lists'))
